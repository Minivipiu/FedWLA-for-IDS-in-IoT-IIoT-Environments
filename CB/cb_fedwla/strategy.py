"""FedWLA Custom Aggregation Strategy Module for CatBoost."""
...
class FedWLA(FedAvg):
    ...
    Estrategia de agregación dinámica basada en incertidumbre, calidad y tamaño.
    """
    ...
    def aggregate_fit(
        ...
        # Clamp and format hyperparameters for CatBoost
        # [Iterations, Depth, Learning Rate]
        aggregated_params[0][0] = max(10, int(np.round(aggregated_params[0][0])))
        aggregated_params[0][1] = max(1, int(np.round(aggregated_params[0][1])))
        aggregated_params[0][2] = max(0.001, float(aggregated_params[0][2]))
