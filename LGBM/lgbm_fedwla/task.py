...
def get_partition(partition_id: int, num_partitions: int, dataset_path: str, imbalance_factor: int):
    ...
    cache_prefix = cache_dir / f"part_lgbm_{dataset_name}_{num_partitions}_{imbalance_factor}_{PARTITION_SEED}"
    lock_file = cache_dir / f"data_prep_lgbm_{dataset_name}_{num_partitions}_{imbalance_factor}_{PARTITION_SEED}.lock"
    ...
    with FileLock(lock_file):
        if not target_cache_file.exists():
            ...
            partitions = create_noniid_partitions_weighted(
                df=data, num_clients=num_partitions, label_col=label_col,
