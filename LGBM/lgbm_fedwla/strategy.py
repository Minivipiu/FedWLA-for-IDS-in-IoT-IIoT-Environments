"""FedWLA Custom Aggregation Strategy Module for LightGBM."""
...
class FedWLA(FedAvg):
    ...
    Adapted for LightGBM hyperparameter sets.
    """
    ...
    def aggregate_fit(
        ...
        aggregated_params[0][2] = max(6, int(np.round(aggregated_params[0][2])))
        aggregated_params[0][3] = max(2, int(np.round(aggregated_params[0][3])))
