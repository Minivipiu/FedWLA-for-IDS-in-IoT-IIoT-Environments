...
PARTITION_SEED = 42
CACHE_VERSION = "noniid_nodup_safe_local_split_v2"
...
def get_partition(partition_id: int, num_partitions: int, dataset_path: str, imbalance_factor: int):
    ...
    cache_prefix = cache_dir / (
        f"part_dt_{CACHE_VERSION}_{dataset_name}_{num_partitions}_{imbalance_factor}_{PARTITION_SEED}"
    )
    lock_file = cache_dir / (
        f"data_prep_dt_{CACHE_VERSION}_{dataset_name}_{num_partitions}_{imbalance_factor}_{PARTITION_SEED}.lock"
    )
...
def calculate_uncertainty(model: DecisionTreeClassifier, X: np.ndarray) -> float:
